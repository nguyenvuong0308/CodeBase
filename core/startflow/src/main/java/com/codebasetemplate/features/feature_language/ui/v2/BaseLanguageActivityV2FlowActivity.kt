...
abstract class BaseLanguageActivityV2FlowActivity : StartFlowActivity<StartflowActivityLanguageV2Binding>() {
    ...
    override fun providerInterAdPlaceName(): List<IAdPlaceName> {
        return if (isFromSetting) {
            listOf(CoreAdPlaceName.FULLSCREEN_BACK_LANGUAGE_SETTING)
        } else {
            emptyList()
        }
    }
    ...
    private fun checkIfNavigationIsNeeded() {
        ...
        val nextIntent = Intent(this, destination).apply {
            ...
            putExtra(StartFlowShortcut.KEY_SHORTCUT_TARGET_SCREEN, targetScreenFromShortCut)
        }
    ...
    private fun showBackInterAdThenFinish() {
        ...
        showInterAd(CoreAdPlaceName.FULLSCREEN_BACK_LANGUAGE_SETTING) {
            finish()
        }
    }
